...
public class LoadTestClient {
    ...
    private void requestClientId() throws IOException, InterruptedException {
        ...
        try {
            conn.outputStream.writeObject(new ClientIdRequest(clientAddress, brokerAddress));
        ...
        try (ServerSocket serverSocket = new ServerSocket(clientAddress.port())) {
            ...
            while (running) {
                try {
                    ...
                    // Check if pool is still active before submitting task
                    if (!shuttingDown && !incomingConnectionsPool.isShutdown()) {
                        incomingConnectionsPool.submit(() -> handleIncomingConnection(socket));
                    } else {
                        try {
                            socket.close();
                    ...
                    // Expected due to SO_TIMEOUT - continue
                    continue;
                } catch (IOException e) {
...
//            }
    ...
    private void performRead(String queueId) throws IOException {
        ...
        try {
            ...
            connection.outputStream.writeObject(new ReadRequest(queueId, clientId, operationId, clientAddress, brokerAddress));
    ...
    private void performWrite(String queueId, Integer value) throws IOException {
        ...
        try {
            ...
            // Send request using cached output stream
            connection.outputStream.writeObject(new WriteRequest(queueId, value, clientId, operationId, clientAddress, brokerAddress));
            connection.outputStream.reset(); // Reset object cache to prevent memory leaks
            connection.outputStream.flush();
